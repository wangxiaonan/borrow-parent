...
import com.borrow.manage.utils.id.impl.IdConverterImpl;
import com.borrow.manage.utils.id.impl.SimpleTimer;
...
public abstract class AbstractIdProvider implements IdProvider {
    ...
    public AbstractIdProvider() {
        idType = IdType.SECONDS;
    }

    public AbstractIdProvider(String type) {
        idType = IdType.parse(type);
    }

    public AbstractIdProvider(long type) {
        idType = IdType.parse(type);
    }

    public AbstractIdProvider(IdType type) {
        idType = type;
    }
    ...
    public Date transTime(final long time) {
        return timer.transTime(expId(time).getTime());
    }
