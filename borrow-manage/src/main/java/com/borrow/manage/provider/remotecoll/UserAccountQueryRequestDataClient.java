...
@Component("userAccountQueryRequestDataClient")
public class UserAccountQueryRequestDataClient extends  DataClient {
    ...
    public String getData(XMap param){
        ...
        try {
            ...
            param.put(PlatformConstant.FundsParam.CONTROL,PlatformConstant.FundsMethod.USER_ACCOUNT_QUERY_REQUEST);
            param.put(PlatformConstant.FundsParam.REQ_TIME, Utility.dateStr());
            String reqParam = JSON.toJSONString(param);
            logger.info("userAccountQueryRequestData_req:url={},params={}",remoteConfig.fundsBaseUrl,reqParam);
            jsonData = doSendRequest(reqParam);
            logger.info("userAccountQueryRequestData_res:result={}",jsonData);

        }catch (Exception e) {
            logger.error("账户查询接口异常",e);
